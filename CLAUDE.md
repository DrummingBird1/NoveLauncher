...
- **Language:** Kotlin only. i18n covers 7 locales — he/en/ar/fr/ru (v9) plus es/de (v9.2) — 353 keys each, kept in lockstep (`sed -n 's/.*<string name="\([^"]*\)".*/\1/p' values-XX/strings.xml | sort` across locales should always diff-empty; verified this way when es/de were added). Enums and `LauncherPage` use `@StringRes Int` displayNameRes (LauncherPage + ThemePreset have a hybrid pattern — built-in entries set the res id, user-created entries keep the typed String). The four NewsSource Hebrew brand names (Walla/Haaretz/Kan/Calcalist) are kept verbatim because they are the outlets' actual names. `AppearanceSettings.appLanguage` now has a real picker UI (Settings → Appearance → language chips) — see Conventions below for why it also calls `.recreate()`.
...
- **Repo root layout (v9.1):** exactly two folders at the true repo root. [android/](android/) is the entire Gradle/Android Studio project (open **this** folder in Android Studio, not the repo root) — root build files, `gradle/`, and the single `:app` module live inside it. [assets/](assets/) holds everything else non-code: design source (`generate_graphics.py`, `graphics/`, `screenshots/`) and [assets/distribution/](assets/distribution/) (Play Store submission text + [assets/distribution/ALTERNATIVE_STORES.md](assets/distribution/ALTERNATIVE_STORES.md), a launch plan for stores other than Google Play). `.github/` stays at the true repo root regardless (GitHub only reads workflows/dependabot config from `<repo-root>/.github/`), as do `CLAUDE.md`/`AGENTS.md` (Claude Code / Codex only auto-load project instructions from the root they're opened in).
...
20. **`AILauncherApp.BACKUP_CHANNEL_ID` is the first `NotificationChannel` this app has ever created** — `ScheduledBackupWorker` is the only current poster. `POST_NOTIFICATIONS` (API 33+) has no runtime-request flow yet; `NotificationManagerCompat.notify()` is wrapped in `catch (_: SecurityException)` so an ungranted permission silently no-ops instead of crashing the worker.
...
| Add a language | new `values-XX/strings.xml` with all 353 keys + add `"XX" to "Native name"` in the `languages` list in [AppearanceSettings.kt](android/app/src/main/java/com/ailauncher/app/ui/screens/AppearanceSettings.kt) |
